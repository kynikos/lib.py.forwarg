...
def parse(parser):
    return parser.parse_args(shlex.split('--opt1 foo'))
...
def test_1_optional_long(_1_optional_argument_long):
    for parser in _1_optional_argument_long:
        assert parse(parser) == _m_argparse.Namespace(opt1='foo')
...
def test_2_optional(_2_optional_arguments):
    for parser in _2_optional_arguments:
        assert parse(parser) == _m_argparse.Namespace(opt1='foo', opt2=None)
